...
function Invoke-PetMenuItem(
    ...
    $pet = Wait-AppElement $Process "圆圆桌面宠物" $true $false 20
    ...
    $menuHandle = Wait-NativeMenu $Process
...
function Wait-PetHidden([System.Diagnostics.Process]$Process, [int]$TimeoutSeconds = 10) {
    $deadline = [DateTime]::UtcNow.AddSeconds($TimeoutSeconds)
    while ([DateTime]::UtcNow -lt $deadline) {
        if ($null -eq (Find-AppElement $Process.Id "圆圆桌面宠物" $true $false)) {
            return $true
        }
        Start-Sleep -Milliseconds 100
    }
...
try {
    ...
    try {
        ...
        [void](Wait-AppElement $candidateProcess "圆圆桌面宠物" $true $false 30)
        ...
        Invoke-PetMenuItem $candidateProcess "隐藏圆圆"
        ...
        [void](Wait-AppElement $candidateProcess "圆圆桌面宠物" $true $false 20)
