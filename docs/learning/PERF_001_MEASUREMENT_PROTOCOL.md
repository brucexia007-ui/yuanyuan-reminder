...
状态：冷温启动、学习页/小黑板、强提醒、活动学习抢占、2 小时学习内存与 pre-GEN 纯解析基线已完成；统一产品的 20,000 卡真实 Tauri CSV 导入/取消/分页、1,000 次答案和 DB 增长已通过开发态候选，正式发布必须由严格门在最新干净提交上重建并生成 `sourceDirty=false` 证据<br>
日期：2026-08-19<br>
...
- 统一产品的用户 CSV 导入器当前边界为 25 MiB/20,000 行，并提供读取、解析和数据库事务内的协作取消。它可以作为本轮用户自备 CSV 的稳定版门，但不冒充尚未冻结的 PACK-001 通用 JSON 内容包结论。
...
- `measure_community_stable_learning_runtime.ps1`：统一产品真实 Tauri 进程内的 20,000 卡 CSV 导入、事务取消、分页、1,000 次答案、数据库增长、统一备份恢复、数据库健康、正式数据句柄隔离和测试根清理。
- 对应的 Node evidence verifier：拒绝陈旧、缺字段、原始样本/摘要不一致或伪正式门报告。

## 当前处置

代码已提供独立 learning-on runtime-QA 构建脚本，避免覆盖其他构建产物。冷/温启动、学习页、小黑板、强提醒、活动学习中强提醒抢占和 2 小时学习内存正式桌面采样已经完成。用户随后明确授权“只解析、只校验、零入库”的 pre-GEN spike，4,533/20,000 JSON/CSV 纯解析正式基线也已完成。统一产品 v1.5.4 开发态候选进一步完成了真实 Tauri 后台的 20,000 卡 CSV 导入、确定性事务中途取消、首/中/末分页、1,000 次答案、DB 增长和统一备份恢复；该次报告因生成时工作区未提交，仅作为候选诊断证据，必须在干净提交上重建并重跑后才能关闭正式门。
...
PERF-001 的统一产品发布判定要求：上述真实 Tauri 规模验收必须绑定最新干净提交；若未来继续 PACK-001，还需另行完成其冻结格式、接近 25 MiB 的真实通用 JSON 包导入和 UI 响应协议，不能用本轮用户 CSV 结论替代。
...
报告绑定应用、fixture、4,533 卡合成内容、初始 SQLite 和脚本 SHA-256，并记录 Windows 10 Pro 25H2 build 26200.8875、24 个逻辑处理器、WebView2 151.0.4129.93 和交流电源。它证明的是隔离 learning-on release 运行时在“完成黑板”稳态下未出现持续内存、句柄或线程增长；20,000 卡导入/分页由后续统一产品规模门覆盖，但本报告仍不覆盖签名生产候选、多 DPI、Narrator、减少动画或独立 24 小时稳定版门。
...
该结果证明冻结预算附近的“纯 Rust 文件读取+解析”有充足余量，并证明文件管线各主要阶段能按固定单位间隔报告进度和协作取消；serde_json/csv 解码输入、Unicode 规范化输入、CSV 分隔符/可选字段扫描与单卡内部自有循环的取消粒度也已分别冻结为 16 KiB。它仍不约束操作系统阻塞在单次 ≤4 MiB 读取调用内的即时取消，也不包含文件选择/替换 token、Tauri IPC、后台任务调度、IPC 进度传递、预览、数据库事务、分页查询或 WebView 响应。后续统一产品规模门独立覆盖生产 CSV 的这些集成路径；本节数字本身不替代该门，也不等同于 PACK-001 通用 JSON 包完成。

## 2026-08-27 统一产品学习规模候选门

`measure_community_stable_learning_runtime.ps1 -AllowDirty` 已在隔离数据根启动真实 v1.5.4 `runtime-qa-learning` Tauri 进程，并通过以下检查：

| 指标 | 开发态候选结果 |
| --- | ---: |
| 导入 / 分页 | 20,000 卡；首、中、末页及越界页通过 |
| 取消 | 第 1,024 次协作检查触发；来源、词包、卡片、复习记录零残留 |
| 答题 | 1,000 次生产事务 |
| 数据库体积 / 答题增量 | 34,046,960 / 1,843,200 bytes，低于 128 MiB / 32 MiB 门 |
| 统一备份恢复 | 恢复 20,000 卡和 1,000 条复习记录 |
| 健康 | `quick_check=ok`；外键违规 0 |
| 隔离与退出 | 正式用户数据未使用；QA 进程无正式数据库句柄；受控退出；测试根已清理 |

开发态证据为 `community-stable-learning-20260827T091218488Z.json`，运行耗时 136,306 ms。它带有 `sourceDirty=true`，因此只证明当前实现路径和门禁可运行；发布判定必须使用提交后的同一脚本重建并生成 `sourceDirty=false` 的新证据。文件选择器自身的“取消”仍由操作系统提供；应用内“停止导入”负责标记后台解析/确认操作并在数据库事务中安全回滚。
