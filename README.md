...
导入的宠物有动作或显示问题？将原 `.yuanyuan-pet`、问题描述和可选录屏交给你的智能体，使用 [宠物包修复提示词](AI_PET_REPAIR_PROMPT.md)。智能体在副本中诊断、修复并提供前后对比，你通过“我的宠物”导入新形象；保留旧版可随时切回，昵称不会自动继承。首次制作同时交付独立的[私人素材工程](pet-template/source-project.template.md)，方便后续修复。

想换成自己的宠物，可以用[一段提示词制作自己的桌面宠物](AI_CUSTOMIZATION_PROMPT.md)。即使电脑上没有本项目，也可将提示词与照片交给支持参考图生成或编辑的 Coding 工具。默认摄影写实，先确认主形象再制作动作，最终导入独立宠物包，保留圆圆主程序和已有数据。
...
目前应用不能直接上传照片自动生成。由支持参考图生成或编辑、能处理本地文件的 AI Coding 工具或素材制作方完成：

1. 准备 3–8 张你有权使用的宠物照片，覆盖正脸、侧脸、全身和标志性花纹；
2. 将照片、宠物名、性格和 [AI 定制提示词](AI_CUSTOMIZATION_PROMPT.md) 交给具备上述能力的 Codex、Kimi Code、WorkBuddy 或素材制作方；
3. 查看原照与主形象的私有对照，确认像自己的宠物且风格合适；默认摄影写实，也可明确指定其他风格。主形象变更时需重新确认；
4. 制作方按 [自定义宠物教程](docs/CUSTOMIZE_YOUR_PET.md) 试做代表动作，再生成、验收完整宠物包；成品在“设置 → 我的宠物”中导入。

完整动作和文件格式见 [宠物包规范](docs/PET_PACK_SPEC.md)。原始照片决定身份；圆圆、饺饺只作为质感和自然动作的参考。保持实际年龄、比例、毛流和花纹，不为可爱而大眼化或玩偶化，不能用一张静态图代替连续动作帧。效果取决于图片工具的参考图能力及实际验收，打包成功不代表写实效果通过。
